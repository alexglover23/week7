...
// create a signout option
document.querySelector('.sign-in-or-sign-out').innerHTML = `
    <a href="#" class="sign-out text-pink-500 underline">Sign out</a>
`

// create the event listener for signout
document.querySelector('.sign-out').addEventListener('click', function(event) {
  event.preventDefault()
  firebase.auth().signOut()
  // refresh the page to clear the todo list from the screen
  document.location.href = 'todo.html'
    ...
    document.querySelector('form').addEventListener('submit', async function(event) {
      ...
      if (todoText.length > 0) {
        let docRef = await db.collection('todos').add({
          text: todoText,
          // assign the todo list collection to a specific user in the database
          userID: user.uid
        })
    ...
    // only display the todo list for the current user
